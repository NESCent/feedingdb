...
class Migration:
    ...
    def forwards(self, orm):
        system = orm['auth.User'].objects.get(username__iexact = 'SYSTEM')
        from datetime import datetime
        now = datetime.now()
         
        orm.AgeUnit.objects.create(label = 'days',   created_by = system, created_at = now, updated_at = now)
        orm.AgeUnit.objects.create(label = 'weeks',  created_by = system, created_at = now, updated_at = now)
        orm.AgeUnit.objects.create(label = 'months', created_by = system, created_at = now, updated_at = now)
        orm.AgeUnit.objects.create(label = 'years',  created_by = system, created_at = now, updated_at = now)
           
    
    def backwards(self, orm):
       for a in orm.AgeUnit.objects.all():
            a.delete()
