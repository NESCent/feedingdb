...
class BoundField(StrAndUnicode):
    ...
    def _display(self):
        return self.as_widget(readonly=True)    	
    ...
    def _value(self):
        """
        return the plain text value of this field
        """
        if not self.form.is_bound:
            v = self.form.initial.get(self.name, self.field.initial)
            if callable(v):
                v = v()
        else:
            if isinstance(self.field, FileField) and self.value is None:
                v = self.form.initial.get(self.name, self.field.initial)
            else:
                v = self.value
        return v
    ...
    def as_widget(self, widget=None, attrs=None, only_initial=False, readonly=False):
        """
