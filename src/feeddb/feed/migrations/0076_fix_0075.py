...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        from existing FKs (session and experiment)
        """
        for session in orm.Session.objects.all():
            session.study = session.experiment.study
            session.save()

        for trial in orm.Trial.objects.all():
            trial.experiment = trial.session.experiment
            trial.study = trial.session.study
            trial.save()
