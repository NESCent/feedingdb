...
class Migration:
    ...
    def forwards(self, orm):
        def location_of_muscle(muscleid):   
            "A helper, mapping old muscle IDs to new location IDs"
            location = orm.AnatomicalLocation.objects.get(tmp_muscleid__exact = muscleid)
            return location
        
        for s in orm.EmgSensor.objects.all():
            s.location_controlled = location_of_muscle(s.muscle.id)
            s.sensor_ptr.location_freetext = None
            s.save()
            s.sensor_ptr.save()
            
        for s in orm.SonoSensor.objects.all():
            s.location_controlled = location_of_muscle(s.muscle.id)
            s.sensor_ptr.location_freetext = None
            s.save()
            s.sensor_ptr.save()
            
        for s in orm.StrainSensor.objects.all():
            s.sensor_ptr.location_freetext = s.sensor_ptr.location
            s.sensor_ptr.save()
            
        for s in orm.ForceSensor.objects.all():
            s.sensor_ptr.location_freetext = s.sensor_ptr.location
            s.sensor_ptr.save()
        
        for s in orm.PressureSensor.objects.all():
            s.sensor_ptr.location_freetext = s.sensor_ptr.location
            s.sensor_ptr.save()
        
        for s in orm.KinematicsSensor.objects.all():
            s.sensor_ptr.location_freetext = s.sensor_ptr.location
            s.sensor_ptr.save()

        #raise Exception('0021 migration -- temporary abort')
