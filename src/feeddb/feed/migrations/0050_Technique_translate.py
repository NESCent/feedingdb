...
class Migration:
    ...
    def forwards(self, orm):
        label2newid = dict(map(lambda(id, lab): (lab, id), TECHNIQUE_CHOICES))

        print "Translation of technique codes (forwards, model ids to enums)"
        oldid2newid = {}
        for t in orm['feed.Technique'].objects.all():    
            oldid = t.id
            label = t.label 
            newid = label2newid.get(label, -1)
            oldid2newid[oldid] = newid 
            print "%i ->\t%i \t(%s)" % (oldid, newid, label)
       
        def translate(modelname):
            for x in orm[modelname].objects.all():
                oldid = x.technique_id
                newid = oldid2newid[oldid]
                x.techn_enum = newid
                x.save()
                #print "%i \tto \t%i" % (oldid, newid)
       
        print "Processing feed.Unit"
        translate('feed.Unit')
            
        print "Processing feed.Setup"
        translate('feed.Setup')

    
    def backwards(self, orm):
        for x in orm['feed.Unit'].objects.all():
            x.techn_enum = None
            x.save()
        for x in orm['feed.Setup'].objects.all():
            x.techn_enum = None
            x.save()
    ...
    models = {
        ...
        'feed.setup': {
            ...
            'techn_enum': ('django.db.models.fields.IntegerField', [], {'null': 'True'}),
