...
class Migration:
    ...
    def forwards(self, orm):
        for e in orm.EmgElectrode.objects.all():
            s = orm.Sensor.objects.get(name = e.name, setup = e.setup)
            e.sensor_id = s.id
            c = orm.Channel.objects.get(name = e.name, setup = e.setup)
            e.channel_id = c.id
            e.save()
#            print "sensor ID = %s, name = %s, setup = %s" % (s.id, s.name, s.setup.id)
#            e.sensor = orm.Sensor.objects.get(name = e.name, setup = e.setup)
#            e.channel = orm.Channel.objects.get(name = e.name, setup = e.setup)
