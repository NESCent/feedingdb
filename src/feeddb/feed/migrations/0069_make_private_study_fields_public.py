...
class Migration(DataMigration):
    ...
    def migrate_fields_on_objects(self, src=None, dest=None):
        dest.pi = src.pi
        dest.organization = src.organization
        dest.lab = src.lab
        dest.funding = src.funding
        dest.approval = src.approval

    def forwards(self, orm):
        for studyprivate in orm.StudyPrivate.objects.all():
            self.migrate_fields_on_objects(src=studyprivate, dest=studyprivate.study)

    def backwards(self, orm):
        for study in orm.Study.objects.all():
            studyprivate = study.studyprivate_set.get()
            self.migrate_fields_on_objects(src=study, dest=studyprivate)
