...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """ Migrate public notes and pi to private """
        for study in orm.Study.objects.all():
            try:
                studyprivate = study.studyprivate
            except ObjectDoesNotExist:
                studyprivate = orm.StudyPrivate()
                now = datetime.datetime.today()
                studyprivate.created_at = now
                studyprivate.updated_at = now
                studyprivate.study = study

            studyprivate.notes = study.notes
            studyprivate.pi = study.pi
            studyprivate.save()

    def backwards(self, orm):
        """ Migrate private notes and pi to public """
        for studyprivate in orm.StudyPrivate.objects.all():
            studyprivate.study.notes = studyprivate.notes
            studyprivate.study.pi = studyprivate.pi
            studyprivate.study.save()
