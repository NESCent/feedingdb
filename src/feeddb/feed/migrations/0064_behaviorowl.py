...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for trial in orm.Trial.objects.all():
            ...
            bp = trial.behavior_primary
            bpo = orm.BehaviorOwl.objects.filter(rdfs_is_class=True, label__icontains=bp.label).first()
            trial.behaviorowl_primary = bpo

            # old behavior_secondary is a CharField
            bs = trial.behavior_secondary
            bso = orm.BehaviorOwl.objects.filter(rdfs_is_class=True, label__icontains=bs).first()
            trial.behaviorowl_secondary = bso
