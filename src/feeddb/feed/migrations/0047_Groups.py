...
class Migration:
    ...
    def forwards(self, orm):
        def ensure_group(group_name):
            "Adds a group, only if it does not already exist."
            gs = orm['auth.Group'].objects.filter(name__iexact = group_name)
            if len(gs) == 0:
                orm['auth.Group'].objects.create(name = group_name)                
        ensure_group('contributors')
        ensure_group('terminologists')
        ensure_group('anonymous')
    
    ## Warning: Since there is no perfect inverse for the above migration, 
    ## the executable code in this backwards() is commented-out, 
    ## to prevent deletion of groups that existed prior to forwards(). 
    ## In particular, running the commented code will remove group-user associations, 
    ## which will not be restored even if forwards() is re-run.     
    ## Uncomment if you understand what you are up to.  
    def backwards(self, orm):
        def ensure_no_group(group_name):
            "Removes a group, if it exists."
            gs = orm['auth.Group'].objects.filter(name__iexact = group_name)
            if len(gs) != 0:
                anon = gs[0]
                anon.delete()
        #ensure_no_group('contributors')
        #ensure_no_group('terminologists')
        #ensure_no_group('anonymous')
