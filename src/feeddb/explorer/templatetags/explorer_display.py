...
def display_file(value):
    ...
    if value in (None, ''):
        return ""
    if is_image(value):
        return u'<a href="%s%s" title="click to view full size image"><span class="glyphicon glyphicon-picture"></span></a>' % (settings.MEDIA_URL,value,)

    return u'<a href="%s%s"><span class="glyphicon glyphicon-picture"></span></a>' % (settings.MEDIA_URL,value,)
